# ===== TERRAFORM CONFIGURATION FOR ZAMAN ASSISTANT =====
...
terraform {
  ...
  backend "s3" {
    bucket         = "zaman-terraform-state"
    key            = "prod/terraform.tfstate"
    region         = "eu-central-1"
    encrypt        = true
    dynamodb_table = "zaman-terraform-locks"
  }
}
...
# ===== LOCALS =====
locals {
  name_prefix = "zaman-${var.environment}"
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        {
          name  = "DATABASE_URL"
          value = "postgresql://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.endpoint}/zaman_db"
        },
        {
          name  = "REDIS_URL"
...
resource "aws_secretsmanager_secret" "openai_key" {
  ...
  description = "OpenAI Hub API Key"
